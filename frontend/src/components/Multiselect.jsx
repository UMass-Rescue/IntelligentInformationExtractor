...
//     const [items, setItems] = useState(['cat 1c1cat 1cat 1cat 1', 'cat 2', 'cat 3', 'cat 4']);

//     const [caseitems, setCaseitems] = useState(['case1, case2, case3']);
...
const Multiselect = ({ value, onChange }) => {
    ...
    const addTag = (item) => {
        setSelected(selectedItems.concat(item));
        setDropdown(false);
        onChange(selectedItems.concat(item).join(', ')); // Pass concatenated string to parent onChange function
    };
    ...
    const removeTag = (item) => {
        const filtered = selectedItems.filter((e) => e !== item);
        setSelected(filtered);
        onChange(filtered.join(', ')); // Pass concatenated string to parent onChange function
    };
            ...
            <div className="autocomplete">
                <div className="w-full flex flex-col items-center mx-auto">
                    <div className="w-full">
                        ...
                        {dropdown && <Dropdown list={value === 'category' ? items : caseItems} addItem={addTag} />}
                    </div>
                </div>
            </div>
        </div>
